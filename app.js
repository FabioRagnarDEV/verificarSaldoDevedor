...
function calcular() {  
    ...
    setTimeout(() => {  
        ...
        const grupoCota = document.getElementById('grupoCota').value; // Grupo e Cota  
        ...
        const totalInicial = fundoComum + taxaAdministrativa + fundoReserva + taxaAdesao; // Soma dos componentes  
        const totalPago = pagoFundoComum + pagoTaxaAdministrativa + pagoFundoReserva + pagoTaxaAdesao; // Soma dos pagos  
        ...
        document.getElementById('result').textContent = resultado; // Mostra o resultado  

        // Exibe a caixa de confirmação  
        document.getElementById('confirmationBox').style.display = 'block';  
    }, 700); // Simula um atraso de 700ms para visualização  
}  

// Função para exibir um alerta personalizado  
function showCustomAlert(message) {  
    document.getElementById('customAlertMessage').textContent = message; // Define a mensagem do alerta  
    document.getElementById('customAlert').style.display = 'flex'; // Exibe o alerta  
}  

// Evento de click para o botão "OK" no alerta  
document.getElementById('customAlertOK').addEventListener('click', function () {  
    document.getElementById('customAlert').style.display = 'none'; // Fecha o alerta ao clicar em OK  
});  

// Função para zerar os campos e resultados  
function zerar() {  
    document.querySelectorAll('.input-group input').forEach(input => input.value = ''); // Limpa os campos  
    document.getElementById('result').textContent = ''; // Limpa o resultado exibido  
    document.getElementById('confirmationBox').style.display = 'none'; // Esconde a caixa de confirmação  
}  